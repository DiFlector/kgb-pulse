<?php
/**
 * API для объединения команд - Организаторы
 */
 ...
 * @param int $mainTeamOid - OID основной команды
 * @param array $teamsToMerge - массив команд для объединения
 * @param int $eventId - ID мероприятия
 * @return array - результат проверки
 */
function checkTeamsCompatibility($db, $mainTeamOid, $teamsToMerge, $eventId) {
    $errors = [];
    $totalParticipants = 0;
    $mainTeamDisciplines = [];
    $mainTeamSex = null;
    ...
    foreach ($teamsToMerge as $teamData) {
        ...
        // Получаем oid команды для объединения
        $teamOid = $db->fetchOne("SELECT oid FROM teams WHERE teamid = ?", [$teamId]);
...
try {
    ...
    if (!$auth->isAuthenticated() || !$auth->hasAnyRole(['Admin', 'Organizer', 'SuperUser'])) {
        sendJson(['success' => false, 'message' => 'Доступ запрещен']);
        return;
    }
    ...
    if (defined('TEST_MODE')) {
        ...
        // В обычном режиме используем JSON из php://input
    ...
    // Минимальная логика для тестов
    if (defined('TEST_MODE')) {
        sendJson([
            'success' => true,
            'message' => 'Команды успешно объединены (тестовый режим)',
            'newTeamId' => 1
        ]);
        return;
    }
    ...
    // Получаем oid основной команды
    $mainTeamOid = $db->fetchOne("SELECT oid FROM teams WHERE teamid = ?", [$mainTeamId]);
    if (!$mainTeamOid || empty($mainTeamOid['oid'])) {
        throw new Exception("Основная команда не найдена");
    }
    $mainTeamOid = $mainTeamOid['oid'];
    
    // Проверяем совместимость команд перед объединением
    $compatibilityCheck = checkTeamsCompatibility($db, $mainTeamOid, $teamsToMerge, $eventId);
    ...
    try {
        ...
        // Для организаторов проверяем, что команды принадлежат их мероприятиям
        if (!$hasFullAccess) {
            $stmt = $pdo->prepare("
            ...
            ");
            $stmt->execute([$mainTeamOid, $eventId, $userId]);
        ...
        foreach ($teamsToMerge as $teamData) {
            ...
            $team = $db->fetchOne("SELECT oid, teamname FROM teams WHERE oid = ?", [$teamOid]);
            if (!$team) {
                continue;
            }
            
            // Для организаторов проверяем права на команду
            if (!$hasFullAccess) {
                $stmt = $pdo->prepare("
                    SELECT COUNT(*) FROM listreg lr
                    JOIN meros m ON lr.meros_oid = m.oid
                    WHERE lr.teams_oid = ? AND m.champn = ? AND m.created_by = ?
                ");
                $stmt->execute([$teamOid, $eventId, $userId]);
                
                if ($stmt->fetchColumn() == 0) {
                    continue; // Пропускаем команды, на которые нет прав
                }
            }
            ...
            foreach ($participants as $participant) {
                $db->execute("
                    ...
                    SET teams_oid = ? 
                    WHERE oid = ?
                ", [$mainTeamOid, $participant['oid']]);
                
                $totalParticipants++;
            }
            
            // Удаляем команду после перемещения участников
            $db->execute("DELETE FROM teams WHERE oid = ?", [$teamOid]);
            $mergedCount++;
        }
