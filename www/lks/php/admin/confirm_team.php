...
try {
    ...
    $stmt = $db->prepare("
        SELECT lr.oid, lr.users_oid, lr.status, u.fio, u.email, m.meroname
        FROM listreg lr
    ...
    // Подтверждаем всех участников команды
    foreach ($participants as $participant) {
        if ($participant['status'] === 'В очереди') {
            $stmt = $db->prepare("
                UPDATE listreg 
    ...
    $stmt = $db->prepare("
        ...
        VALUES (?, ?, ?, NOW())
    ");
    $stmt->execute([$_SESSION['user_id'], "Подтверждение команды {$teamid}", $_SERVER['REMOTE_ADDR'] ?? '']);
    ...
    error_log("Ошибка в confirm_team.php (admin): " . $e->getMessage());
