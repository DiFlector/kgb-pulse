...
?>
    ...
    <script>
        ...
        function displayQueueData(data) {
            // Обрабатываем данные из API администратора
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group-vertical btn-group-sm">
                                ...
                                ${!participant.oplata ? 
                                    `<button class="btn btn-warning btn-sm mb-1" onclick="confirmPayment(${participant.oid})" 
                                            title="Подтвердить оплату">
                                        <i class="fas fa-dollar-sign"></i> Оплата
                                    </button>` : 
                                    `<button class="btn btn-danger btn-sm mb-1" onclick="cancelPayment(${participant.oid})" 
        ...
        // Подтверждение участия администратором (из "В очереди" в "Подтверждён")
        ...
        async function confirmPayment(oid) {
            ...
            try {
                const response = await fetch('/lks/php/admin/confirm_payment.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
        ...
        // Отмена оплаты администратором (только администратор может отменять)
        async function cancelPayment(oid) {
            if (!confirm('Отменить оплату участия?')) {
                return;
            }

            try {
                const response = await fetch('/lks/php/admin/cancel_payment.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
                    body: `registration_id=${oid}&oplata=0`
                });
                
                const result = await response.json();
                
                if (result.success) {
                    showSuccess('Оплата отменена.');
                    loadQueueData(); // Перезагружаем данные
                } else {
                    showError(result.error || result.message || 'Ошибка при отмене оплаты');
                }
            } catch (error) {
                showError('Ошибка соединения: ' + error.message);
                console.error('Error:', error);
        ...
        // Подтверждение команды администратором
        async function confirmTeam(teamid, champn) {
            ...
            try {
                const response = await fetch('/lks/php/admin/confirm_team.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
        ...
        // Подтверждение оплаты команды администратором
        async function confirmTeamPayment(teamid, champn) {
            ...
            try {
                const response = await fetch('/lks/php/admin/confirm_team_payment.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
